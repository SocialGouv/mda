...
const logDebugInformation = (definitions, options = {}) => {
  ...
  if (!silent) {
    // Metrics
    console.log(
      chalk.greenBright(`Generated ${definitions.length} type definition for your Strapi application's schemas.`),
    );

    // Filepath
    const relativePath = path.relative(process.cwd(), filepath);

    console.log(chalk.grey(`The definitions file has been generated here: ${chalk.bold(relativePath)}`));
  }
};
...
const generateSchemasDefinitions = async (options = {}) => {
  const { strapi, outDir = process.cwd(), file = DEFAULT_OUT_FILENAME, verbose = false, silent = false } = options;
